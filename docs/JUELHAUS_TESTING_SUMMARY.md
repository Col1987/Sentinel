...
| `checkout.html`'s email-verification gate has a race condition in its own auth-state check | A dead, unused `onAuthStateChanged` listener sits alongside a synchronous poll of `auth.currentUser` that exits almost immediately (~100ms), before Firebase's auth-state persistence layer has necessarily hydrated. Depending on which side of this race wins on a given page load, an unverified customer proceeding to checkout quickly after registration may bypass the intended verification gate entirely and reach the config/payment flow without ever confirming their email. Severity: medium-high — an inconsistent security control on the purchase flow, not a hard bypass, but the verification requirement cannot currently be relied upon as guaranteed | Confirmed via Playwright trace evidence (not assumed): the same test setup produced both outcomes across different runs, isolating this to a genuine timing race rather than a fixed rule |
...
- Minor admin-tool reliability observation: opening an order directly by ID from the dashboard doesn't always render the order detail modal — confirmed happening on repeated live runs, not a one-off. Falling back to the search box and opening the order from there works reliably every time. Not a functional or security finding — the order data itself is always correct and reachable — just an inconsistency in one specific way of opening it.
...
| An unverified customer can complete a purchase | Race condition in `checkout.html`'s auth-state check: a dead `onAuthStateChanged` listener alongside a synchronous `auth.currentUser` poll that exits before Firebase's auth state has necessarily hydrated | `representative-checkout-completes`, `order-status-progression-and-email-trigger` (surfaced via a CI-only failure investigation, not a dedicated assertion) | **Low — confirmed happening intermittently.** The identical test setup produced both outcomes — gate shown, gate bypassed — across repeated live runs, confirmed via Playwright trace evidence rather than assumed from a single observation |

**Reading this table the way it's meant to be read:** five rows say "Low — confirmed happening." Those five are the actual priority list, regardless of what the raw finding count elsewhere in this document might suggest. Everything marked "High" in this table is a genuine, tested, real-evidence confirmation — not an assumption of safety in the absence of a test.
...
**A second wrong finding, caught the same way.** A test verifying that a custom property name entered at checkout ("Sentinel Alpha Lodge") correctly persists to the admin order record reported a high-severity finding: the admin order showed a generic placeholder name instead. Investigating further (prompted by an unrelated reliability fix to the same admin-lookup code) found the real cause — the test grabbed the first row in a filtered admin table and assumed it was the order just created, with no verification. It wasn't; it was an unrelated order from three weeks earlier, and the "placeholder name" being compared against belonged to that order, not the current one. Once the test was corrected to open the order matching its own checkout email specifically, the same live run showed the property name persisting and displaying correctly, exactly as intended. This finding never reached a formal report — caught during the same investigation that produced it — but is recorded here for the same reason as the multi-property retraction above: an automated finding is only as trustworthy as the record it was actually checked against, and that's worth stating plainly rather than letting the corrected result quietly replace the wrong one.
